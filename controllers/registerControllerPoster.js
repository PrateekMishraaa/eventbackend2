const RegistrationPoster = require("../models/RegistrationPoster");
...
const updateFormDataPoster = async (req, res) => {
  ...
  try {
    ...
    const principalImage = req.files?.principalImage || [];
    ...
    console.log('compressImage',uploadImages);
    
    const compressedPrincipalImage = [];
      await compressImage(principalImage[0].path, principalImage[0].path);
      compressedPrincipalImage.push(principalImage[0].path);
    console.log('principalImage', compressedPrincipalImage)
    if (!email)
      return res.status(400).json({ message: "Email is required in params" });

    let userRegistration = await RegistrationPoster.findOne({ email });

    if (!userRegistration) {
      // ✅ Create new registration if not found
      userRegistration = new RegistrationPoster({
        schoolName,
        teacherName,
        teacherContact,
        email,
      ...
      if (compressedPrincipalImage.length) {
        userRegistration.principalImage.push(...compressedPrincipalImage);
      }
    ...
    userRegistration.quotes = quotes || userRegistration.quotes;
    userRegistration.principalName = principalName || userRegistration.principalName;
...
const getRegistrationByEmailPoster = async (req, res) => {
  try {
    const { email } = req.params;

    const registration = await RegistrationPoster.findOne({ email });
...
const getDataByDistrictPoster = async (req, res) => {
  try {
    const { state, district } = req.body;
    console.log("poster" , state, district);
    ...
    const registrations = await RegistrationPoster.find(query);
...
const getAllRegistrationsDataPoster = async (req, res) => {
  try {
    const registrations = await RegistrationPoster.find();
    res.status(200).json(registrations);
  } catch (error) {
...
const updateEmailAndNumberPoster = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Update the document
    const updatedRegistration = await RegistrationPoster.findByIdAndUpdate(
      id,
      { $set: updateFields },
      {
        new: true,
...
const updatewhatsappstatusPoster = async (req, res) => {
  try {
    const { id } = req.params;
    const { whatsappStatus } = req.body;

    const updated = await RegistrationPoster.findByIdAndUpdate(
      id,
      { whatsappStatus },
      { new: true }
    );
